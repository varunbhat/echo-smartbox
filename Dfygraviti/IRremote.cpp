...
void IRsend::sendOnida(unsigned long data, int nbits) {
  char count=10;
  enableIROut(36);
  mark(8200);
  space(4100);
...
void IRsend::sendOnida1(unsigned long data, int nbits) {
  char count=10;
  enableIROut(38);
  mark(8362);
  space(4300);
...
void IRrecv::setPins(int recvpin)
{
  irparams.recvpin = recvpin;
  irparams.blinkflag = 0;
}
...
#ifdef DEBUG
    Serial.println("Attempting Onida decode");
#endif 
    if (decodeOnida(results)) {
        return DECODED;
    }
...
long IRrecv::decodeOnida(decode_results *results) {
  long data = 0;
  ...
  // Initial mark
  if (!MATCH_MARK(results->rawbuf[offset], ONIDA_HDR_MARK)) {
    return ERR;
  }
  offset++;
  if (!MATCH_SPACE(results->rawbuf[offset], ONIDA_HDR_SPACE)) {
      return ERR;
    }
  offset++;
  while (offset + 1 < irparams.rawlen)
 {
    ...
    if (MATCH_MARK(results->rawbuf[offset], ONIDA_ONE_MARK))
    {
      data = (data << 1) | 1;
      offset++;
      if (!MATCH_SPACE(results->rawbuf[offset], ONIDA_ONE_SPACE))
      	return ERR;	
    } 
    else if (MATCH_MARK(results->rawbuf[offset], ONIDA_ZERO_MARK))
    {
      data <<= 1;
      offset++;
      if (!MATCH_SPACE(results->rawbuf[offset], ONIDA_ZERO_SPACE))
      	return ERR;	
